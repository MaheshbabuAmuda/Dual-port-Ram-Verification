class ram_wr_monitor extends uvm_monitor;
  ...
  // UVM methods
  extern function new(string name = "ram_wr_monitor",
...
// Constructor
function ram_wr_monitor::new(string name = "ram_wr_monitor",
                             uvm_component parent);
...
// Collect write transaction from DUT
...
function void ram_wr_monitor::report_phase(uvm_phase phase);
  ...
  `uvm_info(get_type_name(),
            $sformatf(
              "Report: RAM Write Monitor Collected %0d Transactions",
              m_cfg.mon_rcvd_xtn_cnt),
            UVM_LOW)
